...
public class ProductCardSpecification {
    ...
    public static Specification<ProductCard> hasWarrantyBiggerThan(Short warranty) {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.gt(root.get(ProductCard_.warranty), warranty);
    }

    public static Specification<ProductCard> hasInstallmentPeriodBiggerThan(Short installmentPeriod) {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.gt(root.get(ProductCard_.installmentPeriod), installmentPeriod);
    }

    public static Specification<ProductCard> hasMaxDeliveryTimeLessThan(Short maxDeliveryTime) {
        return (root, query, criteriaBuilder) ->
                criteriaBuilder.lessThan(root.get(ProductCard_.maxDeliveryTime), maxDeliveryTime);
    }
    ...
    public static Specification<ProductCard> buildSpecification(
        ...
        if (Objects.nonNull(warranty)) spec = spec.and(hasWarrantyBiggerThan(warranty));
        if (Objects.nonNull(installmentPeriod)) spec = spec.and(hasInstallmentPeriodBiggerThan(installmentPeriod));
        if (Objects.nonNull(maxDeliveryTime)) spec = spec.and(hasMaxDeliveryTimeLessThan(maxDeliveryTime));
